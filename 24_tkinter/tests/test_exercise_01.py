"""Tests for exercise 01 -- the loop has to turn.

You are meant to read this file: it says exactly what your exercise has to do.

Nothing here opens a visible window. `root.withdraw()` keeps it off the screen and
the event loop works exactly the same -- which is the only reason this module can be
tested at all.
"""
