...
of pretending, and that list is not the less important one.
...
def tool(*command: str) -> tuple[bool, str]:
    ...
    colour their output unless they are told not to.
    """
...
def report() -> None:
    """Print what has been recorded so far, and what no script can record."""
...
def main() -> int:
    ...
    # ---- reading the data -------------------------------------------------
    # From here on every criterion touches your code, so every criterion can raise.
    # This is a feedback channel and not a test suite: an exception has to become a
    # line saying what went wrong, not a traceback in place of the other criteria.
    try:
        readings = list(myreport.load(HERE))
        ...
        # Named, not swallowed. The line says which criterion was being evaluated
        # when your code raised, and the report below stops there.
        stopped = len(results) + 1
        check(f"{stopped:<2} checking stopped here", False, f"{type(exc).__name__}: {exc}")
    ...
    report()
    return 1 if any(not passed for passed, _, _ in results) else 0
